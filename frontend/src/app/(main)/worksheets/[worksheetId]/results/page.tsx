...
export default function Page({ params }: { params: Promise<{ worksheetId: string }> }) {
  ...
  return <Box sx={{ backgroundColor: "rgb(253,251,247)", minHeight: "100vh", py: 5, px: { xs: 2, sm: 4, md: 6 } }}><Box sx={{ maxWidth: 1000, mx: "auto" }}>
    <Button component={Link} href={`/worksheets/${worksheetId ?? ""}`} startIcon={<ArrowBackIcon sx={{ fontSize: 16 }} />} sx={{ color: "rgb(77,69,64)", textTransform: "none", fontSize: 14, mb: 2.5, p: 0, minWidth: 0, "&:hover": { backgroundColor: "transparent", color: INK } }}>Back to worksheet</Button>
