        ...
        {filtered.map((worksheet) => {
          ...
          return <Card key={worksheet.id} component="article" variant="outlined" sx={{ borderColor: BORDER, borderLeft: worksheet.status === "MARKED" ? "3px solid #93A896" : worksheet.status === "SUBMITTED" ? "3px solid #E08A72" : "3px solid #D8B77A", borderRadius: "12px", bgcolor: CARD_BG, boxShadow: "none", px: { xs: 2, sm: 2.75 }, py: { xs: 2, sm: 2.25 }, display: "flex", gap: 2, alignItems: { sm: "center" }, flexWrap: "wrap" }}>
            <Box sx={{ flex: "1 1 340px", minWidth: 0 }}>
              ...
              <Typography component={Link} href={`/worksheets/${worksheet.id}`} sx={{ display: "inline-block", fontFamily: "'Playfair Display', Georgia, serif", fontSize: { xs: 21, sm: 23 }, fontWeight: 500, lineHeight: 1.2, color: INK, textDecoration: "none", "&:hover": { color: "#9E3A24", textDecoration: "underline" } }}>{worksheet.title}</Typography>
            ...
            <Stack direction="row" useFlexGap spacing={1} sx={{ alignItems: "center", flexWrap: "wrap" }}>
              <Button component={Link} href={`/worksheets/${worksheet.id}`} sx={{ minHeight: 40, borderRadius: "9px", border: "1px solid #E4DCD0", color: "#5A544C", px: 2, textTransform: "none", fontWeight: 600, "&:hover": { bgcolor: "#F9F4EC" } }}>Open worksheet</Button>
              {worksheet.status === "ASSIGNED" ? <Button component={Link} href={`/upload?ws=${worksheet.id}`} startIcon={<UploadFileIcon />} sx={{ minHeight: 40, borderRadius: "9px", bgcolor: "#9E3A24", color: "#FFFDFA", px: 2, textTransform: "none", fontWeight: 600, "&:hover": { bgcolor: "#7F2D1D" } }}>Upload work</Button> : null}
              {worksheet.status === "MARKED" ? <Button component={Link} href={`/worksheets/${worksheet.id}/results`} sx={{ minHeight: 40, color: "#9E3A24", px: .5, textTransform: "none", fontWeight: 600 }}>View results</Button> : null}
              {worksheet.status === "SUBMITTED" ? <Typography role="status" sx={{ color: "#9E3A24", fontSize: 12.5, fontWeight: 600 }}>Awaiting tutor review</Typography> : null}
            </Stack>
          </Card>;
        })}
      </Stack>}
