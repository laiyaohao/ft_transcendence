...
export default function ProgressPage() {
  ...
  const load = React.useCallback(async () => {
    ...
    catch (reason) { setError(reason instanceof Error ? reason.message : "Progress could not be loaded."); }
  }, []);
  React.useEffect(() => {
    let current = true;
    const request = async () => {
      try {
        const loaded = await fetchMasteryMap();
        if (current) setData(loaded);
      } catch (reason) {
        if (current) setError(reason instanceof Error ? reason.message : "Progress could not be loaded.");
      }
    };
    void request();
  ...
  return <Box sx={{ minHeight: "100vh", bgcolor: "#F7F4EF", px: { xs: 2.5, sm: 3.75 }, py: 3.75, color: "#2A2622" }}>
    <Box sx={{ maxWidth: 1420, mx: "auto" }}>
      ...
      {!data && !error ? <Card aria-label="Loading mastery map" variant="outlined" sx={{ borderColor: "#EBE4D9", bgcolor: "#FFFDFA", p: 2.5 }}><Skeleton height={34} width="34%" /><Skeleton height={92} sx={{ mt: 1 }} /></Card> : null}
      ...
      {data ? <MasteryMap data={data} heading="Your topic mastery" /> : null}
    </Box>
  </Box>;
}
