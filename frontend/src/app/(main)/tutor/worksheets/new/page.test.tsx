...
vi.mock("@/components/worksheets/WorksheetBuilder", () => ({
  WorksheetBuilder: ({ classId, initialStudentId, onApproved }: {
    classId: number;
    initialStudentId?: number;
    onApproved?: (worksheet: { id: number; sourceClassId?: number; assignments: [] }) => void;
  }) => <><output data-testid="worksheet-target-context">{classId}:{initialStudentId ?? "none"}</output><button onClick={() => onApproved?.({ id: 22, sourceClassId: 12, assignments: [] })}>Complete approval</button></>,
}));
...
describe("GenerateWorksheetPage", () => {
  ...
  it("replaces the generator route with the filtered worksheet index after approval", async () => {
    ...
    await userEvent.setup().click(screen.getByRole("button", { name: "Complete approval" }));
