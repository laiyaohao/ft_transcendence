...
function TutorUploadWizard() {
  ...
  return <Box sx={{ minHeight: "100vh", bgcolor: "#F7F4EF", px: { xs: 2, sm: 4 }, py: 4 }}><Box sx={{ maxWidth: 850, mx: "auto" }}>
    ...
    {step === 1 && <><Typography component="h1" sx={{ fontFamily: "'Playfair Display', Georgia, serif", fontSize: 31, mb: 1 }}>Choose a student worksheet</Typography><Typography sx={{ color: "#6F675E", mb: 2 }}>Select the class, enrolled student, and approved worksheet before uploading their completed pages.</Typography>
      ...
      <Stack direction={{ xs: "column", sm: "row" }} justifyContent="space-between" gap={1} sx={{ mt: 3 }}><Button variant="outlined" sx={secondary} onClick={() => setRetryCount((count) => count + 1)}>Reload</Button><Stack direction="row" gap={1}><Button onClick={() => hasValidSelection && router.push(`/manual-answers?classId=${selectedClassId}&studentId=${selectedStudentId}&worksheetId=${selectedWorksheetId}`)} disabled={!hasValidSelection || loadingClasses || loadingStudents || loadingWorksheets} variant="outlined" sx={secondary}>Enter answers manually</Button><Button onClick={() => setStep(2)} disabled={!hasValidSelection || loadingClasses || loadingStudents || loadingWorksheets} endIcon={<ArrowForwardIcon />} sx={primary}>Upload scan / photo</Button></Stack></Stack></>}
  ...
  return <Box sx={{ minHeight: "100vh", bgcolor: "#F7F4EF", px: { xs: 2, sm: 4 }, py: 4 }}>
    <Box sx={{ maxWidth: 850, mx: "auto" }}>
      ...
      {profile !== null && worksheet !== null ? <>
        ...
        <Typography sx={{ color: "#6F675E", mb: 2 }}>Choose a scan/photo for OCR, or type answers directly. Both methods use the same Tutor review workflow.</Typography>
        ...
        <Stack direction="row" justifyContent="space-between" sx={{ mt: 3 }}>
          ...
          <Stack direction="row" gap={1}><Button component={Link} href={`/manual-answers?worksheetId=${worksheet.id}`} variant="outlined" sx={secondary}>Enter answers manually</Button><Button onClick={() => void submit()} disabled={uploading || pages.length === 0} sx={primary}>{uploading ? "Uploading…" : "Save and continue to OCR review"}</Button></Stack>
        </Stack>
      </> : null}
    </Box>
  </Box>;
