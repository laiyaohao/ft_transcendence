...
import { fetchStudentWorksheet } from "@/services/worksheets";

const navigation = vi.hoisted(() => ({ params: new URLSearchParams("submissionId=55") }));
vi.mock("next/navigation", () => ({ useSearchParams: () => navigation.params, useRouter: () => ({ replace: vi.fn() }) }));
...
vi.mock("@/services/worksheets", () => ({ fetchStudentWorksheet: vi.fn() }));
