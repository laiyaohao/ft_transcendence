...
describe("Student submission document client", () => {
  ...
  it("submits confirmed OCR page mappings to the protected review endpoint", async () => {
    vi.mocked(fetch).mockResolvedValue(new Response(JSON.stringify({ submissionDocumentId: 8, submissionIds: [20], status: "PENDING_REVIEW" }), { status: 201 }));
    await expect(submitOcrForTutorReview(8, [{ extractionId: 12, questionBankId: 30 }])).resolves.toMatchObject({ submissionDocumentId: 8, status: "PENDING_REVIEW" });
    expect(fetch).toHaveBeenCalledWith(expect.stringContaining("/submission-documents/8/submit-for-review"), expect.objectContaining({ method: "POST", body: JSON.stringify({ answers: [{ extractionId: 12, questionBankId: 30 }] }) }));
    await expect(submitOcrForTutorReview(0, [])).rejects.toMatchObject({ status: 400 });
  });
  ...
  it("loads and saves typed answers through the canonical manual-answer contract", async () => {
    ...
    await expect(fetchManualAnswerDraft({ studentId: 7, worksheetId: 42 }))
      .resolves.toMatchObject({ submissionDocumentId: 8, answers: [{ questionBankId: 30, answer: "Evaporation" }] });
    await expect(saveManualAnswers({
      studentId: 7,
      worksheetId: 42,
      answers: [{ questionBankId: 30, answer: "Evaporation" }],
      submit: true,
    })).resolves.toMatchObject({ submissionDocumentId: 8, submissionIds: [20], status: "PENDING_REVIEW" });

    expect(fetch).toHaveBeenNthCalledWith(1,
      expect.stringContaining("/manual-answers?studentId=7&worksheetId=42"),
      expect.objectContaining({ headers: { Authorization: "Bearer student-token" } }));
    expect(fetch).toHaveBeenNthCalledWith(2,
      expect.stringContaining("/manual-answers"),
      expect.objectContaining({ method: "POST", body: JSON.stringify({
        studentId: 7, worksheetId: 42, answers: [{ questionBankId: 30, answer: "Evaporation" }], submit: true,
      }) }));
  });
});
