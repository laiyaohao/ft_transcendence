...
function QuestionCard({ item, selected, onToggle }: { item: QuestionBankItem; selected: boolean; onToggle: () => void }) {
  ...
  return <Card component="article" variant="outlined" sx={{ p: { xs: 2, sm: 2.25 }, borderRadius: "12px", bgcolor: selected ? "#FDF6F3" : "#FFFDFA", borderColor: selected ? "#E0B9AC" : "#EBE4D9", boxShadow: "none", display: "flex", gap: 1.5, alignItems: "flex-start" }}>
    ...
    <Box sx={{ flex: 1, minWidth: 0 }}>
      ...
      <Box sx={{ display: "flex", flexWrap: "wrap", alignItems: "center", justifyContent: "space-between", gap: 1, mt: .8 }}><Typography sx={{ color: "#8B837A", fontSize: 11.5, fontVariantNumeric: "tabular-nums" }}>{item.totalMarks.toFixed(1)} marks · {item.code} · {item.syllabusTopic.nodeType.toLowerCase()}</Typography><Button component={Link} href={`/questions/${item.id}/edit`} size="small" sx={{ minHeight: 32, color: "#9E3A24", textTransform: "none", fontWeight: 500 }}>Edit question</Button></Box>
    </Box>
  </Card>;
}
...
export default function QuestionList({ loadQuestions = fetchTutorQuestions }: QuestionListProps) {
  ...
  const filters = React.useMemo<QuestionBankFilters>(() => ({ topicId, questionType, archiveState, page, size: 12 }), [archiveState, page, questionType, topicId]);
  const load = React.useCallback(async () => {
    setPageData(null); setError(null);
    try { setPageData(await loadQuestions(filters)); }
    catch (reason) { setError(reason instanceof Error ? reason.message : "The question bank could not be loaded. Please try again."); }
