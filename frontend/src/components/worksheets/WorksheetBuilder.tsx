...
export function WorksheetBuilder({ classId, generate = generateWorksheet, generateDiagnostic = generateDiagnosticWorksheet,
  ...
  const submit = async () => {
    ...
    setBusy(true); setError(null); const input = { targetMode, studentIds: targetMode === "STUDENTS" ? selectedStudents : undefined, topicIds: topics, questionCount, dueAt: dueAt || undefined, title: title || undefined, instructions: instructions || undefined };
    try { const response = diagnostic ? await generateDiagnostic(selectedClassId, input, randomKey()) : await generate(selectedClassId, input, randomKey()); if (!response.worksheet) throw new Error(response.message || "Worksheet generation did not produce a draft."); setDraft(response.worksheet); const pages = await Promise.all(topics.map((topicId) => loadQuestions({ topicId, archiveState: "ACTIVE", size: 100 }))); setBank(pages.flatMap((page) => page.items)); } catch (reason) { setError(reason instanceof Error ? reason.message : "Worksheet generation could not be started."); } finally { setBusy(false); }
  };
