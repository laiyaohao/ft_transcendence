...
describe("WorksheetBuilder", () => {
  ...
  it("loads Tutor classes in the generator, selects a class, then generates and approves a draft", async () => {
    ...
    render(<WorksheetBuilder generate={generate} approve={approve} loadClasses={loadClasses} loadSyllabus={async () => syllabus} loadQuestions={questions} onApproved={onApproved} />);
    ...
    expect(onApproved).toHaveBeenCalledWith(expect.objectContaining({ id: 9, status: "APPROVED" }));
  });
  ...
  it("shows an approval error instead of the sent confirmation when the assignment fails", async () => {
    ...
    const onApproved = vi.fn();
    render(<WorksheetBuilder generate={generate} approve={approve} loadClasses={async () => classes} loadSyllabus={async () => syllabus} loadQuestions={questions} onApproved={onApproved} />);
