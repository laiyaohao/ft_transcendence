...
public class MarkingReviewStatusSyncController {
    ...
    @PostMapping @Transactional public ResponseEntity<Void> sync(@RequestHeader(value = "X-Learning-Integration-Key", required = false) String candidate, @RequestBody ReviewState request) {
        ...
        if (request == null || request.submissionId() <= 0 || request.studentId() <= 0 || request.tutorUserId() <= 0 || request.revision() <= 0 || request.reviewState() == null || request.occurredAt() == null) throw new IllegalArgumentException("Review state payload is invalid.");
        ...
        MarkingReviewStatusProjection projection = reviews.findById(request.submissionId()).orElseGet(() -> new MarkingReviewStatusProjection(request.submissionId(), request.tutorUserId(), student, revision, state, request.occurredAt()));
        projection.set(revision, state, request.occurredAt()); reviews.save(projection); return ResponseEntity.noContent().build();
    }
    ...
    record ReviewState(String eventKey, long revision, long submissionId, long tutorUserId, long studentId, String reviewState, LocalDateTime occurredAt) { }
