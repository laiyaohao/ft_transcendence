...
public class AlertGenerationService {
    ...
    public List<AlertResponse> refresh(long tutorId) {
        ...
        for (StudentProfile student : students.findAllByTutorIdOrderByFullNameAsc(
            tutorId
        )) {
    ...
    public void refreshOverdueReviews(long tutorId, LocalDateTime now) {
        ...
        List<MarkingReviewStatusProjection> overdueReviews = reviews
            .findByTutorIdAndStateAndRequestedAtLessThanEqualOrderByRequestedAtAsc(
                tutorId,
                MarkingReviewStatusProjection.State.PENDING_REVIEW,
                overdueBefore
            );
        ...
        for (MarkingReviewStatusProjection review : overdueReviews) {
            ...
            createIfAbsent(
                ...
                "This submission has awaited tutor review since "
    ...
    public void refreshStudent(long tutorId, StudentProfile student) {
        if (student == null
            ...
            || !student.getTutorId().equals(tutorId)) {
    ...
    private List<TutorAlert.AlertStatus> activeAlertStatuses() {
        return List.of(
            TutorAlert.AlertStatus.OPEN,
            TutorAlert.AlertStatus.ACKNOWLEDGED
        );
    }
    ...
    private boolean hasWeakTopicMastery(MasteryRecord record) {
        return record.getScore().compareTo(LOW_MASTERY_THRESHOLD) < 0
            && record.getAttemptCount() > 0;
    }

    private void createWeakTopicAlert(
        long tutorId,
        StudentProfile student,
        MasteryRecord record
    ) {
        int attemptCount = record.getAttemptCount();
        String deduplicationKey = "weak-topic:"
            + student.getId()
            + ":"
        ...
        String message = (
            "%s mastery after %d approved attempt%s; "
                + "the alert threshold is below 70%%."
        ).formatted(
            percent(record.getScore()),
            attemptCount,
            attemptCount == 1 ? "" : "s"
        );
    ...
    private void createRepeatedDiagnosticAlerts(
        long tutorId,
        StudentProfile student
    ) {
        Map<String, List<MasteryDiagnosticEvidence>> diagnosticsByRecordAndType =
            diagnostics.findByStudentProfileIdOrderByCreatedAtDescIdDesc(student.getId())
                .stream()
                .collect(Collectors.groupingBy(
                    diagnostic -> diagnostic.getMasteryRecord().getId()
                        + ":"
                        + diagnostic.getMistakeType()
                ));

        for (List<MasteryDiagnosticEvidence> occurrences :
            diagnosticsByRecordAndType.values()) {
    ...
    private void createRepeatedDiagnosticAlert(
        long tutorId,
        StudentProfile student,
        List<MasteryDiagnosticEvidence> occurrences
    ) {
        ...
        String deduplicationKey = "repeated-diagnostic:"
            + student.getId()
            + ":"
            + record.getSyllabusTopic().getId()
            + ":"
        ...
        String message = "%d tutor-confirmed %s records were captured for this topic."
    ...
    public List<AlertResponse> list(long tutorId, List<TutorAlert.AlertStatus> statuses) {
        requirePositive(tutorId);
        return alerts.findAllByTutorIdAndAlertStatusInOrderByCreatedAtDescIdDesc(
            tutorId,
            statuses
    ...
    private AlertResponse mutate(long tutorId, long alertId, boolean resolve) {
        TutorAlert alert = alerts.findByIdAndTutorId(alertId, tutorId)
